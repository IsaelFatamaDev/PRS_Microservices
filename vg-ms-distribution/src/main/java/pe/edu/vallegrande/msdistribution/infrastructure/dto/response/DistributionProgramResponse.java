package pe.edu.vallegrande.msdistribution.infrastructure.dto.response;

import com.fasterxml.jackson.annotation.JsonFormat;
import lombok.Builder;
import lombok.Value;
import lombok.extern.jackson.Jacksonized;

import java.time.Instant;
import java.time.LocalDate;

/**
 * DTO de respuesta para el programa de distribución.
 * Contiene todos los detalles para ser consumidos por el cliente.
 * 
 * @version 1.0
 */
@Value
@Builder
@Jacksonized
public class DistributionProgramResponse {

    /** ID único del programa. */
    String id;

    /** ID de la organización. */
    String organizationId;

    /** Código de programa autogenerado. */
    String programCode;

    /** ID del horario. */
    String scheduleId;

    /** ID de la ruta. */
    String routeId;

    /** ID de la zona. */
    String zoneId;

    /** ID de la calle. */
    String streetId;

    /** Fecha del programa. */
    @JsonFormat(pattern = "yyyy-MM-dd")
    LocalDate programDate;

    /** Hora planificada de inicio. */
    String plannedStartTime;

    /** Hora planificada de fin. */
    String plannedEndTime;

    /** Hora real de inicio. */
    String actualStartTime;

    /** Hora real de fin. */
    String actualEndTime;

    /** Estado actual (ACTIVE, INACTIVE, CON_AGUA, SIN_AGUA). */
    String status;

    /** Usuario responsable. */
    String responsibleUserId;

    /** Observaciones. */
    String observations;

    /** Fecha de creación. */
    Instant createdAt;

    /** Fecha de última actualización. */
    Instant updatedAt;
}
